...
try {
  ...
  outer: for (const file of files) {
    ...
    if (extension === '.xls' || extension === '.xlsx') {
      ...
      const { rows: flowmeters_entry_exists_query_result } = await client.query(
        flowmeters_entry_exists_query,
        [platform_id, report_date_id]
      );
      ...
      inner: for (let i = 14; i < rows.length; i++) {
        ...
        if (row[2] === null) {
          ...
          const { rows: daily_general_comments_previous_entry_query_result } =
            await client.query(daily_general_comments_previous_entry_query, [
              field_id,
              platform,
              report_date_id,
            ]);
        ...
        if (
          ...
          last_gas_well_test_date = processDateValue(last_gas_well_test_date);
          last_lab_date = processDateValue(last_lab_date);
        }
        //

        const liquid_ton = +String(row[20]).replace(/,/g, '.');
        const oil_ton = +String(row[21]).replace(/,/g, '.');
        const water_ton = +String(row[22]).replace(/,/g, '.');
        const total_gas = +String(row[24]).replace(/,/g, '.');
        const gaslift_gas_wt = +String(row[25]).replace(/,/g, '.');
        const water_cut = +String(row[27]).replace(/,/g, '.');
        ...
        //   'WHERE well_id = $1 ' +
        //   'AND (SELECT COUNT(*) = 0 FROM laboratory_results AS lr WHERE well_id = $2 AND ((lr.last_lab_date::date - wt.well_test_date::date) BETWEEN 0 AND 1)) ' +
        //   'AND (wt.well_test_date::date - CURRENT_DATE) >= 7 ' +
        //   'ORDER BY wt.well_test_date DESC LIMIT 10';

        // const {rows: well_test_lab_result_not_exist_list} = await client.query(well_test_lab_result_not_exist_query, [well_id, well_id])
